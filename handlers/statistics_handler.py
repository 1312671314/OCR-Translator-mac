...
class StatisticsHandler:
    ...
    def _write_text_section(self, f, provider, stats, ui_lang, use_polish_format):
        ...
        median_duration_val = trans_stats['median_duration']
        formatted_duration = f'{median_duration_val:.3f} {duration_suffix}'.replace('.', ',') if use_polish_format else f'{median_duration_val:.3f}{duration_suffix}'
        f.write(f"{median_duration_label}: {formatted_duration}\n")
        ...
        words_per_min_val = trans_stats['words_per_minute']
        formatted_wpm = f'{words_per_min_val:.2f}'.replace('.', ',') if use_polish_format else f'{words_per_min_val:.2f}'
        f.write(f"{words_per_minute_label}: {formatted_wpm}\n")
