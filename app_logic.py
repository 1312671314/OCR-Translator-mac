                ...
                try:
                    ...
                    if area:
                        ...
                        if width > 0 and height > 0:
                            from screenshot_utils import capture_screen_region
    ...
    def toggle_translation(self):
        ...
        if self.is_running:
            ...
            try:
                ...
                # Debug: 检查 overlay 状态
                log_debug(f"DEBUG: source_overlay = {self.source_overlay}")
                if self.source_overlay:
                    log_debug(f"DEBUG: source_overlay type = {type(self.source_overlay)}")
                    try:
                        log_debug(f"DEBUG: source_overlay.winfo_exists() = {self.source_overlay.winfo_exists()}")
                    except Exception as e:
                        log_debug(f"DEBUG: source_overlay.winfo_exists() failed: {e}")
