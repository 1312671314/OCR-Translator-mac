...
def _select_screen_area_interactive(app, prompt_text, is_target=False):
    ...
    try:
        ...
        sel_win.configure(bg="black", cursor="crosshair")
        ...
        canvas = tk.Canvas(sel_win, highlightthickness=0, bg="black")
        ...
        def on_press(e):
            ...
            import sys
            # macOS 上 stipple 不生效，使用半透明填充
            if is_target:
                color = app.target_colour_var.get()
                rect_item = canvas.create_rectangle(
                    e.x, e.y, e.x, e.y, 
                    outline=color, 
                    width=4,
                    fill=color,
                    stipple="gray25" if sys.platform != "darwin" else ""
                )
            else:
                color = app.source_colour_var.get()
                rect_item = canvas.create_rectangle(
                    e.x, e.y, e.x, e.y, 
                    outline=color, 
                    width=4,
...
def create_source_overlay_om(app):
    ...
    try:
        log_debug(f"OverlayManager: Creating source overlay with area: {app.source_area}, color: {app.source_colour_var.get()}")
        app.source_overlay = ResizableMovableFrame(app.root, app.source_area, bg_color=app.source_colour_var.get(), title="")
        log_debug(f"OverlayManager: ResizableMovableFrame created: {app.source_overlay}")
        
        app.source_overlay.attributes("-alpha", 0.7)
        log_debug("OverlayManager: Alpha attribute set")
        
        app.source_overlay.update_color(app.source_colour_var.get())
        log_debug("OverlayManager: Color updated")

        should_be_visible = app.config['Settings'].getboolean('source_area_visible', fallback=False)
        log_debug(f"OverlayManager: should_be_visible = {should_be_visible}")
        ...
        log_debug(f"OverlayManager: Traceback: {traceback.format_exc()}")
        app.source_overlay = None
